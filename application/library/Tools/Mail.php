...
class Tools_Mail
{
    protected $_option = array(
            ...
            'charset' => 'utf-8',
            'username' => 'service@p2pbuy.net',
            'password' => 'p2pbuyCllagyygy',
            );
    // ///////////////////////////////////////////////
    // PROPERTIES, PUBLIC
    // ///////////////////////////////////////////////
        ...
        if ($this->Sender != '' && strlen(ini_get('safe_mode')) < 1) {
            ...
            if ($this->SingleTo === true && count($toArr) > 1) {
                foreach ($toArr as $key => $val) {
                    $rt = @mail($val, 
                            $this->EncodeHeader(
                                    $this->SecureHeader($this->Subject)), $body, 
                            $header, $params);
                }
            } else {
                $rt = @mail($to, 
    ...
    {
        if ($this->smtp == NULL) {
            $this->smtp = new Tools_SMTP();
        }
        
        $this->smtp->do_debug = $this->SMTPDebug;
        $hosts = explode(';', $this->_option['host']);
        $index = 0;
        ...
        while ($index < count($hosts) && $connection == false) {
            ...
            if ($this->smtp->Connect(
                    ((! empty($this->SMTPSecure)) ? $this->SMTPSecure . '://' : '') .
                ...
                $connection = true;
                if ($this->SMTPAuth) {
                    if (! $this->smtp->Authenticate($this->_option['username'], 
                            $this->_option['password'])) {
        ...
        switch ($this->message_type) {
            case 'plain':
                ...
                $result .= sprintf("Content-Type: %s; charset=\"%s\"", 
                        $this->ContentType, $this->_option['charset']);
                break;
            case 'attachments':
            /* fall through */
            case 'alt_attachments':
        ...
        switch ($this->message_type) {
            ...
            case 'alt_attachments':
                ...
                $result .= $this->GetBoundary($this->boundary[2], '', 
                        'text/plain', '') . $this->LE; // Create
                                                                                                             // text
                                                                                                             // body
                $result .= $this->EncodeString($this->AltBody, $this->Encoding);
                $result .= $this->LE . $this->LE;
                $result .= $this->GetBoundary($this->boundary[2], '', 
                        'text/html', '') . $this->LE; // Create
                                                                                                            // the
                                                                                                            // HTML
                                                                                                            // body
                $result .= $this->EncodeString($this->Body, $this->Encoding);
    ...
    /*
     * Returns a formatted header line. @access private @return string
     */
    ...
    function EncodeHeader ($str, $position = 'text')
    {
        ...
        $maxlen = 75 - 7 - strlen($this->_option['charset']);
        /*
        ...
        $encoded = preg_replace('/^(.*)$/m', 
                " =?" . $this->_option['charset'] . "?$encoding?\\1?=", $encoded);
            ...
            foreach ($images[2] as $i => $url) {
                ...
                $message = preg_replace(
                        "/" . $images[1][$i] . "=\"" . preg_quote($url, '/') .
                                 "\"/Ui", $images[1][$i] . "=\"" . $cid . "\"", 
                                $message);
                $this->AddEmbeddedImage($url, md5($filename), $filename, 
                        'base64', $mimeType);
            }
        }
        $this->IsHTML(true);
