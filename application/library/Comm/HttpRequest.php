...
class Comm_HttpRequest {
    ...
    public function set_debug($debug, $log_formatter = null) {
        ...
        if ($log_formatter != null) {
            if ($log_formatter instanceof Comm_Log_Formatter) {
                ...
                throw new Comm_Exception_Program('log_fomatter must be Comm_Log_Formatter');
            }
        }
    }
    ...
    public function curl_init() {
        ...
        $ch = Comm_HttpRequestPool::get_curl($this->get_host_id());
    ...
    public function send() {
        ...
        Comm_HttpRequestPool::reset_curl_state($this->get_host_id(), $this->get_curl_id());
        $this->reset_ch();
    ...
    public function get_curl_cli() {
        if (!$this->debug) {
            throw new Comm_Exception_Program("cann't get info when debug disable");
        }
    ...
    public static function urlencode_raw($input) {
        if (is_array($input)) {
            return array_map(array('Comm_HttpRequest', 'urlencode_raw'), $input);
        } else if (is_scalar($input)) {
    ...
    public static function urlencode_rfc3986($input) {
        if (is_array($input)) {
            return array_map(array('Comm_HttpRequest', 'urlencode_rfc3986'), $input);
        } else if (is_scalar($input)) {
